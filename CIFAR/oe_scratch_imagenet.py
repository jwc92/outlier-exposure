...
if args.dataset == 'cifar10':
    train_data_in = dset.CIFAR10('/data/private/OOD+RL/outlier-exposure/CIFAR/datasets', train=True, transform=train_transform)
    test_data = dset.CIFAR10('/data/private/OOD+RL/outlier-exposure/CIFAR/datasets', train=False, transform=test_transform)
    num_classes = 10
else:
...
train_out_dict = {'dataset': 'ImageNet32', 
      'path': 'datasets',
      'batch_size': 128,
      'n_workers': 8,
      'split': 'training', 
      'augmentation': {'hflip': {'p': 0.5}, 
                       'vflip': {'p': 0.5},
                       'rotate': True}
                 } 
train_loader_out = get_dataloader(train_out_dict)
...
with open(os.path.join(args.save, args.dataset + calib_indicator + '_' + args.model +
                                  '_oe_scratch_imagenetepoch150_training_results.csv'), 'w') as f:
...
for epoch in range(start_epoch, args.epochs):
    ...
    torch.save(net.state_dict(),
               os.path.join(args.save, args.dataset + calib_indicator + '_' + args.model +
                            '_oe_scratch_imagenetepoch150_epoch_' + str(epoch) + '.pt'))
    # Let us not waste space and delete the previous model
    prev_path = os.path.join(args.save, args.dataset + calib_indicator + '_' + args.model +
                             '_oe_scratch_imagenetepoch150_epoch_' + str(epoch - 1) + '.pt')
    if os.path.exists(prev_path): os.remove(prev_path)

    # Show results

    with open(os.path.join(args.save, args.dataset + calib_indicator + '_' + args.model +
                                      '_oe_scratch_imagenetepoch150_training_results.csv'), 'a') as f:
